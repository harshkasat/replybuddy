    ...
    return (
        <div className={cn(
            ...
            <AnimatePresence>
                {isTyping && (
                    <motion.div 
                        className="fixed bottom-8 mx-auto transform -translate-x-1/2 backdrop-blur-lg bg-white/90 dark:bg-neutral-800/90 rounded-full px-4 py-2 shadow-lg border border-neutral-200 dark:border-neutral-700"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 20 }}
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-medium text-neutral-700 dark:text-white/90 mb-0.5">zap</span>
                            </div>
                            <div className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300">
                            <span>Thinking</span>
                            <TypingDots /> {/* TypingDots will also need theme adjustment */}
                            </div>
                        </div>
                    </motion.div>
                )}
...
function TypingDots() {
  // const isDarkMode = /* determine if dark mode is active, e.g., via context or prop */;
  // For simplicity, if you're using Tailwind's class strategy and have the 'dark' class on a parent:
  return (
    <div className="flex items-center ml-1">
      {[1, 2, 3].map((dot) => (
        <motion.div
          key={dot}
          // Use Tailwind's dark variant for background
          ...
          // The boxShadow is tricky with inline styles for theming.
          // Option 1: Conditionally apply style (requires knowing the theme state here)
          // style={isDarkMode ? { boxShadow: "0 0 4px rgba(255, 255, 255, 0.3)" } : { boxShadow: "0 0 4px rgba(0, 0, 0, 0.1)" }}
          // Option 2: Use Tailwind shadow utility if a suitable one exists or create custom utility.
          // For now, the background change will be the most impactful. You can add a subtle Tailwind shadow:
          // className="... shadow-sm dark:shadow-glow-white (custom)"
        />
      ))}
    </div>
  );
